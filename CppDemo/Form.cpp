...
void Form::on_btnStopRobot_clicked()
{
    PrintLog(QString::asprintf("send to %s:%hu: StopRobot()", m_RobotConnector.GetIp().c_str(), m_RobotConnector.GetPort()));
    std::thread thd([this]{
        std::string ret = m_Dashboard.StopRobot();
        PrintLog(QString::asprintf("Receive From %s:%hu: %s", m_RobotConnector.GetIp().c_str(),
                                   m_RobotConnector.GetPort(), ret.c_str()));
    });
    thd.detach();
...
void Form::on_btnJointMovJ_clicked()
{
    ...
    PrintLog(QString::asprintf("send to %s:%hu: JointMovJ(%s)", m_RobotConnector.GetIp().c_str(),
                               m_RobotConnector.GetPort(),pt.ToString().c_str()));
    std::thread thd([=]{
...
void Form::ShowDataResult()
{
    ...
    {//第一次填充数据，免得用的时候一个一个输入
        ui->lineEditJ1->setText(QString::asprintf("%.3f", m_Feedback.GetFeedbackData().QActual[0]));
        ui->lineEditJ2->setText(QString::asprintf("%.3f", m_Feedback.GetFeedbackData().QActual[1]));
        ui->lineEditJ3->setText(QString::asprintf("%.3f", m_Feedback.GetFeedbackData().QActual[2]));
        ui->lineEditJ4->setText(QString::asprintf("%.3f", m_Feedback.GetFeedbackData().QActual[3]));
        ui->lineEditJ5->setText(QString::asprintf("%.3f", m_Feedback.GetFeedbackData().QActual[4]));
        ui->lineEditJ6->setText(QString::asprintf("%.3f", m_Feedback.GetFeedbackData().QActual[5]));
    }
